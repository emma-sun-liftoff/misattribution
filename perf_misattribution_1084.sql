...
, sum(IF(custom_event_id = COALESCE(install__ad_click__impression__bid__campaign_target_event_id, reeng_click__impression__bid__campaign_target_event_id) AND first_occurrence,1,0)) AS target_events_first
...
--dt >= '2023-07-03T00' AND dt < '2023-07-04T07'
...
, sum(IF(from_iso8601_timestamp(at) - from_iso8601_timestamp(install_at) < INTERVAL '7' DAY, CAST(customer_revenue_micros AS double)/1000000, 0)) AS customer_revenue_d7
, sum(IF(from_iso8601_timestamp(at) - from_iso8601_timestamp(install_at) < INTERVAL '7' DAY, target_events, 0)) AS target_events_d7
, sum(IF(from_iso8601_timestamp(at) - from_iso8601_timestamp(install_at) < INTERVAL '7' DAY, target_events_first, 0)) AS target_events_first_d7
, sum(IF(from_iso8601_timestamp(at) - from_iso8601_timestamp(install_at) < INTERVAL '7' DAY, CAST(capped_customer_revenue_micros AS double)/1000000, 0)) AS capped_customer_revenue_d7
, sum(IF(from_iso8601_timestamp(at) - from_iso8601_timestamp(install_at) < INTERVAL '7' DAY, CAST(squared_capped_customer_revenue AS double), 0)) AS squared_capped_customer_revenue_d7
